	...
	window.codex = {
		...
		initNavToggleSubmenus: function() {
			...
			for ( let i = 0; i < navTOGGLE.length; i++ ) {
				window.codex.initEachNavToggleSubmenu( navTOGGLE[ i ] );
				window.codex.initEachNavToggleSubmenuInside( navTOGGLE[ i ] );
			}
		},
		initEachNavToggleSubmenu: function( nav ) {
			...
			for ( let i = 0; i < SUBMENUS.length; i++ ) {
				...
				if ( dropdown ) {
					...
					dropdownBtn.setAttribute( 'aria-label', ( dropdown_label ? codexConfig.screenReader.expandOf + ' ' + dropdown_label : codexConfig.screenReader.expand ) );
					...
					dropdownBtn.addEventListener( 'click', function( e ) {
						e.preventDefault();
						window.codex.toggleSubMenu( e.target.closest('li') );
					} );
		
					// Clean up the toggle if a mouse takes over from keyboard.
					parentMenuItem.addEventListener( 'mouseleave', function( e ) {
						window.codex.toggleSubMenu( e.target, false );
					} );
					...
					parentMenuItem.querySelector( 'a' ).addEventListener( 'focus', function( e ) {
						...
						for ( let j = 0; j < parentMenuItemsToggled.length; j++ ) {
							if ( parentMenuItem !== parentMenuItemsToggled[ j ] ) {
								window.codex.toggleSubMenu( parentMenuItemsToggled[ j ], false );
							}
						}
					} );
					...
					SUBMENUS[ i ].addEventListener( 'keydown', function( e ) {
						...
						if ( 9 === e.keyCode ) {
							if ( e.shiftKey ) {
								...
								if ( window.codex.isfirstFocusableElement (SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
									window.codex.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
								}
								// Means we're tabbing out of the end of the submenu.
							} else if ( window.codex.islastFocusableElement( SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
								window.codex.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
							}
						}
			...
			for ( let i = 0; i < SUBMENUPARENTS.length; i++ ) {		
				...
				SUBMENUPARENTS[ i ].addEventListener( 'mouseenter', function( e ) {
					if ( SUBMENUPARENTS[ i ].querySelector( 'ul.sub-menu' ) ) {
						...
						var off = window.codex.getOffset( elm );
						var l = off.left;
		...
		toggleSubMenu: function( parentMenuItem, forceToggle ) {
			...
			*/
			if ( parentMenuItemToggled ) {
				...
				toggleButton.setAttribute( 'aria-label', ( dropdown_label ? codexConfig.screenReader.expandOf + ' ' + dropdown_label : codexConfig.screenReader.expand ) );
				...
				for ( let i = 0; i < subMenuItemsToggled.length; i++ ) {
					window.codex.toggleSubMenu( subMenuItemsToggled[ i ], false );
				}
			} else {
				...
				for ( let i = 0; i < parentMenuItemsToggled.length; i++ ) {
					window.codex.toggleSubMenu( parentMenuItemsToggled[ i ], false );
				}

				// Toggle "on" the submenu.
				parentMenuItem.classList.add( 'menu-item--toggled-on' );
				subMenu.classList.add( 'toggle-show' );
				toggleButton.setAttribute( 'aria-label', ( dropdown_label ? codexConfig.screenReader.collapseOf + ' ' + dropdown_label : codexConfig.screenReader.collapse ) );
			}
		},
		/**
		...
		toggleDrawer: function( element, changeFocus ) {
			...
			window.codex.toggleAttribute( toggle, 'aria-expanded', 'true', 'false' );
			if ( target.classList.contains('show-drawer') ) {
				...
				document.body.classList.remove( 'codex-scrollbar-fixer' );
				setTimeout(function () {
					target.classList.remove('show-drawer');
					var drawerEvent = new Event('codex-drawer-closed');
					...
					if ( toggle.dataset.setFocus && changeFocus ) {
						...
						if ( focusElement ) {
							...
							if ( focusElement.hasAttribute( 'aria-expanded') ) {
								window.codex.toggleAttribute( focusElement, 'aria-expanded', 'true', 'false' );
							}
						}
					}
				}, duration);
				...
				if ( toggle.dataset.toggleBodyClass ) {
					...
					if ( toggle.dataset.toggleBodyClass.includes( 'showing-popup-drawer-' ) ) {
						...
						document.body.classList.add( 'codex-scrollbar-fixer' );
					}
				}
				setTimeout(function () {
					target.classList.add('active');
					var drawerEvent = new Event('codex-drawer-opened');
					window.dispatchEvent(drawerEvent);
					if ( toggle.dataset.setFocus && changeFocus ) {
						var focusElement = document.querySelector(toggle.dataset.setFocus);
		...
		initToggleDrawer: function() {
			...
			for ( let i = 0; i < drawerTOGGLE.length; i++ ) {
				drawerTOGGLE[ i ].addEventListener('click', function( event ) {
					...
					window.codex.toggleDrawer( drawerTOGGLE[ i ] );
				} );
			}
				...
				if ( event.keyCode === 27 ) {
					if ( document.querySelectorAll( '.popup-drawer.show-drawer.active' ) ) {
						...
						document.querySelectorAll( '.popup-drawer.show-drawer.active' ).forEach(function ( element ) {
							window.codex.toggleDrawer( document.querySelector('*[data-toggle-target="' + element.dataset.drawerTargetString + '"]' ) );
						} );
					}
				}
			  });
			...
			document.addEventListener( 'click', function (event) {
				...
				if ( target === modal ) {
					window.codex.toggleDrawer(document.querySelector('*[data-toggle-target="' + modal.dataset.drawerTargetString + '"]'));
				}
		...
		initMobileToggleSub: function() {
			...
			modalMenus.forEach( function( modalMenu ) {
				...
				if ( activeMenuItem ) {
					window.codex.findParents( activeMenuItem, 'li' ).forEach( function( element ) {
						var subMenuToggle = element.querySelector( '.drawer-sub-toggle' );
						if ( subMenuToggle ) {
							window.codex.toggleDrawer( subMenuToggle, true );
						}
					} );
				}
			} );
			...
			for ( let i = 0; i < drawerSubTOGGLE.length; i++ ) {
				drawerSubTOGGLE[ i ].addEventListener('click', function (event) {
					event.preventDefault();
					window.codex.toggleDrawer( drawerSubTOGGLE[ i ] );
				} );
			}
		},
		/**
		...
		initMobileToggleAnchor: function() {
			...
			for ( let i = 0; i < menuLink.length; i++ ) {
				menuLink[ i ].addEventListener('click', function (event) {
					window.codex.toggleDrawer( mobileModal.querySelector( '.menu-toggle-close' ), false );
				} );
			}
		},
		/**
		...
		initTransHeaderPadding: function() {
			...
			var updateHeroPadding = function( e ) {
				header
				if ( codexConfig.breakPoints.desktop <= window.innerWidth ) {
		...
		initStickyHeader: function() {
			var desktopSticky = document.querySelector( '#main-header .codex-sticky-header' ),
				mobileSticky  = document.querySelector( '#mobile-header .codex-sticky-header' ),
				wrapper = document.getElementById( 'wrapper' ),
				proSticky = document.querySelectorAll( '.codex-pro-fixed-above' ),
				proElements = document.querySelectorAll( '.codex-before-wrapper-item' ),
				activeSize = 'mobile',
				lastScrollTop = 0,
				activeOffsetTop = 0;
				if ( parseInt( codexConfig.breakPoints.desktop ) < window.innerWidth ) {
					activeSize = 'desktop';
					if ( desktopSticky ) {
						desktopSticky.style.position = 'static';
						activeOffsetTop = window.codex.getOffset( desktopSticky ).top;
						desktopSticky.style.position = null;
					}
				} else {
					if ( mobileSticky ) {
						mobileSticky.style.position = 'static';
						activeOffsetTop = window.codex.getOffset( mobileSticky ).top;
						mobileSticky.style.position = null;
					}
				}
				var updateSticky = function( e ) {
					var activeHeader;
					var offsetTop = window.codex.getOffset( wrapper ).top;
					if ( document.body.classList.toString().includes( 'boom_bar-static-top' ) ) {
						var boomBar = document.querySelector( '.boom_bar' );
						offsetTop = window.codex.getOffset( wrapper ).top - boomBar.offsetHeight;
					}
					if ( proElements.length ) {
						...
						offsetTop = window.codex.getOffset( wrapper ).top - proElementOffset;
					}
					if ( proSticky.length ) {
						var proOffset = 0;
						...
						offsetTop = window.codex.getOffset( wrapper ).top + proOffset;
					}
					if ( codexConfig.breakPoints.desktop <= window.innerWidth ) {
					...
					if ( codexConfig.breakPoints.desktop <= window.innerWidth ) {
						if ( activeSize === 'mobile' ) {
							activeOffsetTop = window.codex.getOffset( activeHeader ).top;
							activeSize = 'desktop';
				...
				if ( desktopSticky || mobileSticky ) {
					...
					if ( document.body.classList.contains( 'woocommerce-demo-store' ) && document.body.classList.contains( 'codex-store-notice-placement-above' ) ) {
		...
		getTopOffset: function( event = 'scroll' ) {
			if ( event === 'load' ) {
				...
				var desktopSticky = document.querySelector( '#main-header .codex-sticky-header:not([data-reveal-scroll-up="true"])' ),
					mobileSticky  = document.querySelector( '#mobile-header .codex-sticky-header:not([data-reveal-scroll-up="true"])' );
			}
			...
			return Math.floor( activeScrollOffsetTop + activeScrollAdminOffsetTop + Math.floor( codexConfig.scrollOffset ) );
		},
		scrollToElement: function( element, history, event = 'scroll' ) {
			history = (typeof history !== 'undefined') ? history : true;
			var offsetSticky = window.codex.getTopOffset( event );
		...
		anchorScrollToCheck: function( e, respond ) {
			...
			e.preventDefault();
			window.codex.scrollToElement( targetAnchor );
		},
		/**
		...
		initStickySidebarWidget: function() {
			...
			var offsetSticky = window.codex.getTopOffset(),
			...
			if ( window.location.hash != '' ) {
				...
				if ( element ) {
					window.setTimeout( function() {
						window.codex.scrollToElement( element, false, 'load' );
					}, 100 );
				}
			}
				...
				try {
					...
					if ( targetURL.pathname === window.location.pathname ) {
						element.addEventListener( 'click', function( e ) {
							window.codex.anchorScrollToCheck( e );
						} );
					}
				} catch (error) {
		...
		init: function() {
			window.codex.initNavToggleSubmenus();
			window.codex.initToggleDrawer();
			window.codex.initMobileToggleAnchor();
			window.codex.initMobileToggleSub();
			window.codex.initOutlineToggle();
			window.codex.initStickyHeader();
			window.codex.initStickySidebar();
			window.codex.initStickySidebarWidget();
			window.codex.initTransHeaderPadding();
			window.codex.initAnchorScrollTo();
			window.codex.initScrollToTop();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.codex.init );
	} else {
