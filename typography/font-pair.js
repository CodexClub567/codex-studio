...
class FontPairModal extends Component {
	constructor() {
		...
		this.state = {
			isVisible: false,
			confirm: '',
			fonts: ( codexCustomizerControlsData.fontPairs ? codexCustomizerControlsData.fontPairs : [] ),
		};
	}
	render() {
		const toggleVisible = () => {
		...
		return (
			<div className={'codex-font-pair-wrap'}>
				<Button className={ 'codex-font-pair-btn' } label={ __( 'Choose a font', 'codex' ) } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
					<Dashicon icon="portfolio" />
				</Button>
				{ this.state.isVisible && (
					<Popover position="bottom left" inline={true} className="codex-font-pair-popover codex-customizer-popover" onClose={ toggleClose }>
						<h2 style={{ textAlign:'center' }}>{ __( 'Select a Font Pairing', 'codex' ) }</h2>
						<ButtonGroup className="kt-font-pair-group" aria-label={ __( 'Select a Font Pair', 'codex' ) }>
							{ map( this.state.fonts, ( { hfont, bfont, hv, img, name } ) => {
								return (
									this.state.confirm === name ? (
										...
										>
											{ __( 'Confirm Change Settings?', 'codex' ) }
										</Button>
									) : (
										<Button
	...
	updateValues( bodyFont, headingFont ) {
		...
		var event = new CustomEvent('codexRemoteUpdateFonts', {
			'detail': 'typography'
		});
