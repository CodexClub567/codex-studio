...
import codexWebfontLoader from '../common/font-loader.js';
...
class TypographyComponent extends Component {
	constructor() {
		...
		this.state = {
			...
			activeFont: [],
			value: value,
			fontVars: ( codexCustomizerControlsData.gfontvars ? codexCustomizerControlsData.gfontvars : [] ),
			customFontVars: ( codexCustomizerControlsData.cfontvars ? codexCustomizerControlsData.cfontvars : [] ),
		};
	...
	render() {
		...
		const fontFamilyTab = (
			<Fragment>
				<div className="codex-font-family-search">
					<TextControl
						type="text"
				...
				<div className="codex-font-family-list-wrapper">
					<ButtonGroup id={ this.controlParams.id } className="codex-font-family-list" aria-label={ __( 'Font Family List' ) }>
		...
		const fontStyleTab = (
			<Fragment>
				<div className="codex-font-variant-list-wrapper">
					<ButtonGroup id={ this.controlParams.id + '-style' } className="codex-font-variant-list" aria-label={ __( 'Font Style List' ) }>
			...
			<Fragment>
				<div class="codex-range-control">
					<ResponsiveControl
						...
						controlLabel={ __( 'Font Size', 'codex' ) }
						tooltip={ false }
					>
						<RangeControl
							allowReset={ true }
						...
						{ this.controlParams.sizeUnits && (
							<div className="codex-select-units">
				...
				<div class="codex-range-control">
					<ResponsiveControl
						...
						controlLabel={ __( 'Line Height', 'codex' ) }
						tooltip={ false }
					>
						<RangeControl
							allowReset={ true }
				...
				<div class="codex-range-control">
					<ResponsiveControl
						deviceSize={ deviceIndex }
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Letter Spacing', 'codex' ) }
						tooltip={ false }
					>
						<RangeControl
							allowReset={ true }
				...
				<div class="codex-range-control codex-transform-controls">
					<span className="customize-control-title">{ __( 'Transform' ) }</span>
					<ButtonGroup className="codex-radio-container-control codex-radio-icon-container-control">
			...
			<Fragment>
				{
					...
					<span className="customize-control-title">
						<Tooltip text={ __( 'Reset Values', 'codex' ) }>
							<Button
								className="reset codex-reset"
								onClick={ () => {
		...
		if ( this.state.value.family === '-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' ) {
			fontFamilyLabel = __( 'System Default', 'codex' );
		}
		if ( this.state.value.family === 'var(--global-heading-font-family, inherit)' ) {
			fontFamilyLabel = __( 'Inherit Heading', 'codex' );
		}
		if ( this.state.value.family === 'var(--global-body-font-family, inherit)' ) {
			fontFamilyLabel = __( 'Inherit Body', 'codex' );
		}
		return (
			<div ref={ this.anchorNodeRef } className="codex-control-field codex-typography-control-wrap">
				<div className="codex-typography-control">
					{ controlLabel }
					<div className="codex-typography-controls">
						{ this.state.isVisible && 'family' === this.controlParams.options && (
							<Popover position="top right" inline={true} className="codex-popover-color codex-popover-typography codex-customizer-popover" onClose={ toggleClose }>
								<div className="codex-popover-typography-single-item">
									{ fontFamilyTab }
								</div>
							</Popover>
						) }
						{ this.state.isVisible && ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Popover position="top left" inline={true} className="codex-popover-color codex-popover-typography codex-customizer-popover" onClose={ toggleClose }>
								<TabPanel className="codex-popover-tabs codex-typography-tabs codex-background-tabs"
									...
									tabs={ [
										{
											name: 'font',
											title: __( 'Font', 'codex' ),
											className: 'codex-font-typography',
										},
										{
											name: 'style',
											title: __( 'Style', 'codex' ),
											className: 'codex-style-typography',
										},
										{
											name: 'size',
											title: __( 'Size', 'codex' ),
											className: 'codex-size-typography',
										},
									] }>
									{
										( tab ) => {
						...
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Font', 'codex' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'codex-typography-family-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('font') } }>
										{ fontFamilyLabel }
									</Button>
								</div>
							</Tooltip>
						...
						{ ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Style', 'codex' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'codex-typography-style-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('style') } }>
										{ this.state.value.variant ? this.state.value.variant : __( 'inherit', 'codex' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'size' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Size', 'codex' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'codex-typography-size-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('size') } }>
										{ currentSize ? currentSize + this.state.value.sizeType : __( 'inherit', 'codex' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ __( 'Show Preview Text', 'codex' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'codex-typography-preview-indicate' } onClick={ () => { this.state.isPreviewVisible ? toggleClosePreview() : toggleVisiblePreview() } }>
				...
				{ this.state.value.google && (
					<codexWebfontLoader config={ configVariants } >
					</codexWebfontLoader>
				) }
				{ this.state.isPreviewVisible && (
					<div className="codex-preview-font" style={ { 
						...
						{ __( 'Design is not just what it looks like and feels like. Design is how it works.', 'codex' ) }
					</div>
				) }
			</div>
		);
	...
	linkRemoteUpdate() {
		if ( 'base_font' === this.controlParams.id || 'heading_font' === this.controlParams.id ) {
			let self = this;
			document.addEventListener( 'codexRemoteUpdateFonts', function( e ) {
