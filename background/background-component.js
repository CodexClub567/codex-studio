...
import codexGradientPicker from '../gradient-control/index.js';
import codexColorPicker from '../common/color-picker';
...
class BackgroundComponent extends Component {
	constructor(props) {
		...
		let defaultParams = {
			...
			repeat: {
				'no-repeat': {
					name: __( 'No Repeat', 'codex' ),
				},
				'repeat': {
					name: __( 'Repeat', 'codex' ),
				},
				'repeat-x': {
					name: __( 'Repeat-X', 'codex' ),
				},
				'repeat-y': {
					name: __( 'Repeat-y', 'codex' ),
				},
			},
			size: {
				auto: {
				...
				contain: {
					name: __( 'Contain', 'codex' ),
				},
			},
			attachment: {
				scroll: {
					name: __( 'Scroll', 'codex' ),
				},
				fixed: {
					name: __( 'Fixed', 'codex' ),
				},
	...
	render() {
		...
		const toggleVisible = () => {
			const updateColors = JSON.parse( this.props.customizer.control( 'codex_color_palette' ).setting.get() );
		...
		const controlLabel = (
			<Fragment>
				{ this.state.currentDevice !== 'desktop' && (
					<Tooltip text={ __( 'Reset Device Values', 'codex' ) }>
						<Button
		...
		if( this.state.supportGradient ) {
			tab_options = [
				{
					name: 'color',
					title: __( 'Color', 'codex' ),
					className: 'codex-color-background',
				},
				{
					name: 'gradient',
					title: __( 'Gradient', 'codex' ),
					className: 'codex-gradient-background',
				},
				{
					name: 'image',
					title: __( 'Image', 'codex' ),
					className: 'codex-image-background',
				},
			];
		} else {
			tab_options = [
		...
		return (
			<div className="codex-control-field codex-background-control">
				<ResponsiveControl
					onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
					controlLabel={ controlLabel }
				>
					<div className="codex-background-picker-wrap">
						{ this.state.isVisible && (
							<Popover position="top left" className="codex-popover-color codex-customizer-popover" inline={ true } onClose={ maybeToggleClose }>
								<TabPanel className="codex-popover-tabs codex-background-tabs"
												...
												if ( 'image' === tab.name ) {
													tabout = (
														<>
															...
															<span className="customize-control-title">{ __( 'Background Repeat', 'codex' ) }</span>
															...
															<span className="customize-control-title">{ __( 'Background Size', 'codex' ) }</span>
															<ButtonGroup className="codex-radio-container-control">
															...
															<span className="customize-control-title">{ __( 'Background Attachment', 'codex' ) }</span>
															<ButtonGroup className="codex-radio-container-control">
													...
													tabout = (
														<>
															{ this.state.isVisible && this.state.refresh && (
																<div className="codex-background-color-wrap">
																	<codexColorPicker
																		//presetColors={ [] }
						...
						<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Background', 'codex' ) }>
							<div className="background-button-wrap">
								<Button className={ 'codex-background-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
									{ ( undefined === this.state.value[ this.state.currentDevice ] || undefined === this.state.value[ this.state.currentDevice ].type || this.state.value[ this.state.currentDevice ].type === 'color' ) && (
										<Fragment>
											<ColorIndicator className="codex-advanced-color-indicate" colorValue={ getColorPreviewValue( this.state.currentDevice ) } />
											...
											{ showGradientPreview( this.state.currentDevice ) &&
												<ColorIndicator className="codex-advanced-color-indicate" colorValue={ getGradientPreviewValue( this.state.currentDevice ) } />
											}
										</Fragment>
									) }
	...
	preventClose() {
		let self = this;
		document.addEventListener( 'codexOpenMediaModal', function(e) {
