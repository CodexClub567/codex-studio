...
class BuilderComponent extends Component {
	...
	focusPanel( item ) {
		if ( undefined !== this.props.customizer.section( 'codex_customizer_' + item ) ) {
			this.props.customizer.section( 'codex_customizer_' + item ).focus();
		}
	}
	focusItem( item ) {
	...
	render() {
		return (
			<div className={ `codex-control-field codex-builder-items${ ( this.controlParams.rows.includes( 'popup' ) ? ' codex-builder-items-with-popup' : '' ) }` }>
				...
				<div className="codex-builder-row-items">
	...
	// linkFocusButtons() {
	// 	this.props.control.container.on( 'click', '.codex-builder-areas .codex-builder-item', function( e ) {
	// 		e.preventDefault();
