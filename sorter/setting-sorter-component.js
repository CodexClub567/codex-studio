...
class SorterComponent extends Component {
	constructor() {
		...
		let defaultParams = {
			...
			imageSizes: {
				thumbnail: {
					name: __( 'Thumbnail', 'codex' ),
				},
				medium: {
					name: __( 'Medium', 'codex' ),
				},
				medium_large: {
					name: __( 'Medium Large', 'codex' ),
				},
				large: {
					name: __( 'Large', 'codex' ),
				},
	...
	render() {
		const controlLabel = (
			<Fragment>
				{/* <Tooltip text={ __( 'Reset Value', 'codex' ) }>
					<Button
		...
		return (
			<div className="codex-control-field codex-sorter-items codex-post-title-sorter">
				...
				<div className="codex-sorter-row">
					{ this.controlParams.sortable && (
						<ReactSortable animation={100} onStart={ () => this.onDragStop() } onEnd={ () => this.onDragStop() } group={ this.controlParams.group } className={ `codex-sorter-drop codex-sorter-sortable-panel codex-meta-sorter codex-sorter-drop-${ this.controlParams.group }` } handle={ '.codex-sorter-item-panel-header' } list={ theItems } setList={ ( newState ) => this.onDragEnd( newState ) } >
					...
					{ ! this.controlParams.sortable && (
						<div className={ `codex-sorter-drop codex-sorter-sortable-panel codex-sorter-no-sorting codex-sorter-drop-${ this.controlParams.group }` } >
							{ currentList.map( ( item, index ) => {
								return <ItemComponent
									key={ item }
									item={ item }
