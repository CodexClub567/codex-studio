...
import codexColorPicker from './color-picker';
...
class ColorControl extends Component {
	...
	render() {
		...
		return (
			<div className="codex-color-picker-wrap">
				...
				{ showingGradient && (
					<Fragment>
						{ this.state.isVisible && (
							<Popover position={ position } inline={true} anchor={( undefined !== this.props?.controlRef?.current ? this.props.controlRef.current : undefined )} className="codex-popover-color codex-popover-color-gradient codex-customizer-popover" onClose={ toggleClose }>
				...
				{ ! showingGradient && (
					<Fragment>
						{ this.state.isVisible && this.state.refresh && (
							<Popover position="top right" inline={true} anchor={( undefined !== this.props?.controlRef?.current ? this.props.controlRef.current : undefined )} className="codex-popover-color codex-customizer-popover" onClose={ toggleClose }>
								<codexColorPicker
									color={ ( this.state.isPalette && this.state.palette.palette && this.state.palette.palette[parseInt(this.state.color.slice(-1), 10 ) - 1] ? this.state.palette.palette[parseInt(this.state.color.slice(-1), 10 ) - 1 ].color : this.state.color ) }
									onChange={ ( color ) => this.onChangeState( color, '' ) }
									onChangeComplete={ ( color ) => this.onChangeComplete( color, '' ) }
						...
						{ this.state.isVisible && ! this.state.refresh &&  (
							<Popover position={ position } inline={true} anchor={ ( undefined !== this.props?.controlRef?.current ? this.props.controlRef.current : undefined ) } className="codex-popover-color codex-customizer-popover" onClose={ toggleClose }>
								<codexColorPicker
									//presetColors={ [] }
									color={ ( this.state.isPalette && this.state.palette[ this.state.activePalette ] && this.state.palette[ this.state.activePalette ][parseInt(this.state.color.slice(-1), 10 ) - 1] ? this.state.palette[ this.state.activePalette ][parseInt(this.state.color.slice(-1), 10 ) - 1 ].color : this.state.color ) }
									onChange={ ( color ) => this.onChangeState( color, '' ) }
				...
				<Tooltip text={ this.props.tooltip ? this.props.tooltip : __( 'Select Color', 'codex' ) }>
					<div className="color-button-wrap">
						<Button className={ 'codex-color-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<ColorIndicator className="codex-advanced-color-indicate" colorValue={ ( this.state.isPalette ? 'var(--global-' + this.props.color + ')' : this.props.color ) } />
