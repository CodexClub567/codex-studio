if ( codexgooglefonts === undefined ) {
	var codexgooglefonts = [];
}
const {
	Component,
} = wp.element;
...
class codexWebfontLoader extends Component {
	constructor(props) {
		super( props );
	...
	addFont( font ) {
		if ( ! codexgooglefonts.includes( font ) ) {
			codexgooglefonts.push( font );
		}
	};
	...
	loadFonts() {
		...
		if ( ! codexgooglefonts.includes( this.props.config.google.families[ 0 ] ) ) {
			WebFont.load( {
				...this.props.config,
...
codexWebfontLoader.propTypes = {
	config: PropTypes.object.isRequired,
	children: PropTypes.element,
	onStatus: PropTypes.func.isRequired,
};

codexWebfontLoader.defaultProps = {
	onStatus: noop,
};

export default codexWebfontLoader;
