...
use codex_Control_Builder;
use codex_Control_Builder_Tabs;
use Customizer_Sanitize;
use WP_Customize_Control;
use LearnDash_Settings_Section;
use codex\WebFont_Loader;
...
class Theme_Customizer {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_codex_flush_fonts_folder', array( $this, 'ajax_delete_fonts_folder' ) );
	...
	public function convert_beaver_custom_fonts() {
		$beaver_fonts = apply_filters( 'codex_theme_add_custom_fonts', array() );
		if ( ! empty( $beaver_fonts ) && is_array( $beaver_fonts ) ) {
			add_filter(
				'codex_theme_custom_fonts',
	...
	public function ajax_delete_fonts_folder() {
		// Check request.
		if ( ! check_ajax_referer( 'codex-local-fonts-flush', 'nonce', false ) ) {
			wp_send_json_error( 'invalid_nonce' );
		}
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( 'invalid_permissions' );
		}
		if ( class_exists( '\codex\WebFont_Loader' ) ) {
			$font_loader = new \codex\WebFont_Loader( '' );
	...
	public function add_woocommerce_choices() {
		if ( class_exists( 'woocommerce' ) ) {
			...
			$options['cart'] = array(
				'name'    => esc_html__( 'Cart', 'codex' ),
				'section' => 'codex_customizer_cart',
			);
			...
			$options['mobile-cart'] = array(
				'name'    => esc_html__( 'Cart', 'codex' ),
				'section' => 'codex_customizer_mobile_cart',
			);
	...
	public function customizer_custom_widget_areas( $section_args, $section_id, $sidebar_id ) {
		if ( 'footer1' === $sidebar_id || 'footer2' === $sidebar_id || 'footer3' === $sidebar_id || 'footer4' === $sidebar_id || 'footer5' === $sidebar_id || 'footer6' === $sidebar_id ) {
			$section_args['panel'] = 'codex_customizer_footer';
		}
		return $section_args;
	}
	/**
	...
	public function override_defaults( $wp_customize ) {
		 ...
		 * Move Site Identity
		 */
		$wp_customize->get_control( 'site_icon' )->section = 'codex_customizer_site_identity';
		...
		if ( isset( $wp_customize->selective_refresh ) ) {
			...
			$wp_customize->selective_refresh->add_partial(
				...
				array(
					...
					'render_callback'     => 'codex\site_branding',
				)
			);
		}

		if ( ! empty( $opt_name_key = apply_filters( 'codex_settings_key_custom_mapping', '', 'custom_logo' ) ) ) {
			...
			$wp_customize->add_setting(
				...
				array(
					...
					'default'           => isset( $setting['default'] ) && $control_class !== 'codex_Control_Background' ? $setting['default'] : '',
					'sanitize_callback' => array( 'codex\Customizer_Sanitize', 'codex_sanitize_option' ),
				)
			);
		...
		self::$live_control['codex_global_palette'] = $palette_live;
	...
	public function register_controls( $wp_customize ) {
		...
		require_once $path . 'class-codex-control-blank.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-color.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-range.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-switch.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-radio-icon.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-multi-radio-icon.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-builder.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-available.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-color-palette.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-background.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-border.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-borders.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-typography.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-title.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-focus-button.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-color-link.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-text.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-textarea.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-measure.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-editor.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-sorter.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-social.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-contact.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-check-icon.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-select.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-row.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-tab.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-codex-control-shadow.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	}
	/**
	 * Add Upgrade Section
	 *
	...
	public function upgrade_section( $wp_customize ) {
		if ( ! class_exists( 'codex_Theme_Pro' ) ) {
			$path = get_template_directory() . '/inc/customizer/custom-controls/';
			require_once $path . 'class-codex-section-pro.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			$wp_customize->register_section_type( \codex_Section_Pro::class );
			$wp_customize->add_section(
				new \codex_Section_Pro(
					$wp_customize,
					'codex_pro_upgrade',
					array(
						'title'    => esc_html__( 'More Options Available in codex Pro', 'codex' ),
						'pro_link' => esc_url( \codex\codex()->get_pro_url( 'https://www.codexwp.com/codex-theme/premium/', 'https://www.codexwp.com/codex-theme/premium/', 'in-app', 'theme-customizer' ) ),
						'priority' => 1,
					)
				)
			);
			$wp_customize->add_control(
				new \codex_Control_Blank(
					$wp_customize,
					'pro-notice',
					array(
						'label'    => '',
						'section'  => 'codex_pro_upgrade',
						'settings' => array(),
						'priority' => 0,
					)
				)
	...
	public static function get_panels() {
		...
		if ( is_null( self::$panels ) ) {
			$panels = array(
				'design' => array(
					'title'    => __( 'Colors & Fonts', 'codex' ),
					'priority' => 18,
				),
				'header' => array(
					'title'    => __( 'Header', 'codex' ),
					'priority' => 20,
				),
				'footer' => array(
					'title'    => __( 'Footer', 'codex' ),
					'priority' => 22,
				),
				'general' => array(
					'title'    => __( 'General', 'codex' ),
					'priority' => 23,
				),
				'all_posts' => array(
					'title'    => __( 'Posts/Pages Layout', 'codex' ),
					'priority' => 24,
				),
			);
			if ( class_exists( 'SFWD_LMS' ) ) {
				$extra_learn['learndash'] = array(
					'title'    => __( 'LearnDash', 'codex' ),
					'priority' => 25,
				);
				$panels = array_merge(
					$panels,
	...
	public static function get_sections() {
		...
		if ( is_null( self::$sections ) ) {
			$sections = array(
				'site_identity' => array(
					'title'    => __( 'Site Identity', 'codex' ),
					'priority' => 25,
				),
				'general_layout' => array(
					'title'    => __( 'Layout', 'codex' ),
					'panel'    => 'general',
					'priority' => 7,
				),
				'sidebar' => array(
					'title'    => __( 'Sidebar', 'codex' ),
					'panel'    => 'general',
					'priority' => 8,
				),
				'sidebar_design' => array(
					'title'    => __( 'Sidebar Design', 'codex' ),
					'panel'    => 'general',
					'priority' => 8,
				),
				'general_colors' => array(
					'title'    => __( 'Colors', 'codex' ),
					'panel'    => 'design',
					'priority' => 9,
				),
				'general_buttons' => array(
					'title'    => __( 'Buttons', 'codex' ),
					'panel'    => 'design',
					'priority' => 10,
				),
				'general_typography' => array(
					'title'    => __( 'Typography', 'codex' ),
					'panel'    => 'design',
					'priority' => 11,
				),
				'general_image' => array(
					'title'    => __( 'Images', 'codex' ),
					'panel'    => 'general',
					'priority' => 11,
				),
				'scroll_up' => array(
					'title'    => __( 'Scroll To Top', 'codex' ),
					'panel'    => 'general',
					'priority' => 12,
				),
				...
				'general_social' => array(
					'title'    => __( 'Social Links', 'codex' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_comments' => array(
					'title'    => __( 'Comments', 'codex' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_performance' => array(
					'title'    => __( 'Performance', 'codex' ),
					'panel'    => 'general',
					'priority' => 21,
				),
				'breadcrumbs' => array(
					'title'    => __( 'Breadcrumbs', 'codex' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_404' => array(
					'title'    => __( '404 Page Layout', 'codex' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				...
				'header_layout' => array(
					'title'    => __( 'Header Layout', 'codex' ),
					'panel'    => 'header',
					'priority' => 8,
				),
				'header_builder' => array(
					'title'    => __( 'Header Builder', 'codex' ),
					'panel'    => 'header',
					'priority' => 9,
				),
				'header_top' => array(
					'title'    => __( 'Top Row', 'codex' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_main' => array(
					'title'    => __( 'Main Row', 'codex' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_bottom' => array(
					'title'    => __( 'Bottom Row', 'codex' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_popup' => array(
					'title'    => __( 'Header Off Canvas', 'codex' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				...
				'primary_navigation' => array(
					'title'    => __( 'Primary Navigation', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'secondary_navigation' => array(
					'title'    => __( 'Secondary Navigation', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'dropdown_navigation' => array(
					'title'    => __( 'Dropdown Navigation', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_navigation' => array(
					'title'    => __( 'Mobile Navigation', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'mobile_button' => array(
					'title'    => __( 'Header Mobile Button', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_html' => array(
					'title'    => __( 'Header Mobile HTML', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'header_html' => array(
					'title'    => __( 'HTML Editor', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'cart' => array(
					'title'    => __( 'Header Cart', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'cart_design' => array(
					'title'    => __( 'Header Cart Design', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'mobile_cart' => array(
					'title'    => __( 'Mobile Header Cart', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'header_search' => array(
					'title'    => __( 'Header Search', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'transparent_header' => array(
					'title'    => __( 'Transparent Header', 'codex' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'footer_layout' => array(
					'title'    => __( 'Footer Layout', 'codex' ),
					'panel'    => 'footer',
					'priority' => 8,
				),
				'footer_builder' => array(
					'title'    => __( 'Footer Builder', 'codex' ),
					'panel'    => 'footer',
					'priority' => 9,
				),
				'footer_top' => array(
					'title'    => __( 'Footer Top', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_html' => array(
					'title'    => __( 'Footer Copyright HTML', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				'footer_social' => array(
					'title'    => __( 'Footer Social', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_navigation' => array(
					'title'    => __( 'Footer Navigation', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				'page_layout' => array(
					'title'    => __( 'Page Layout', 'codex' ),
					'panel'    => 'all_posts',
					'priority' => 3,
				),
				'search' => array(
					'title'    => __( 'Search Results', 'codex' ),
					'priority' => 24,
				),
				'search_design' => array(
					'title'    => __( 'Search Results', 'codex' ),
				...
				'post_layout' => array(
					'title'    => __( 'Single Post Layout', 'codex' ),
					'panel'    => 'all_posts',
					'priority' => 7,
				),
				...
				// 'custom_post' => array(
				// 	'title'    => __( 'Custom Post Types', 'codex' ),
				// 	'panel'    => 'all_posts',
				// 	'priority' => 24,
				// ),
			);
			if ( ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION, '10.6.2', '>' ) ) || version_compare( substr( get_bloginfo( 'version' ), 0, 3 ), '5.8', '>=' ) ) {
				$blocks_extra['sidebar-widgets-footer1'] = array(
					'title'    => __( 'Widget Footer 1 Settings', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer2'] = array(
					'title'    => __( 'Widget Footer 2 Settings', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer3'] = array(
					'title'    => __( 'Widget Footer 3 Settings', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer4'] = array(
					'title'    => __( 'Widget Footer 4 Settings', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer5'] = array(
					'title'    => __( 'Widget Footer 5 Settings', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer6'] = array(
					'title'    => __( 'Widget Footer 6 Settings', 'codex' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-header1'] = array(
					'title'    => __( 'Widget Header Settings', 'codex' ),
					'panel'    => 'header',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-header2'] = array(
					'title'    => __( 'Widget Area Settings', 'codex' ),
					'panel'    => 'header',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-product-filter'] = array(
					'title'    => __( 'Catalog Off Canvas Sidebar Settings', 'codex' ),
					'panel'    => 'woocommerce',
					'priority' => 18,
				);
			...
			if ( class_exists( 'woocommerce' ) ) {
				$extra_woo['woocommerce_product_catalog'] = array(
					'title'    => __( 'Product Catalog', 'codex' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				...
				$extra_woo['product_layout'] = array(
					'title'    => __( 'Single Product Layout', 'codex' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				...
				$extra_woo['account_layout'] = array(
					'title'    => __( 'My Account Layout', 'codex' ),
					'panel'    => 'woocommerce',
					'priority' => 24,
				);
				$extra_woo['woocommerce_store_notice_design'] = array(
					'title'    => __( 'Store Notice', 'codex' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				$sections = array_merge(
			...
			if ( class_exists( 'SFWD_LMS' ) ) {
				...
				$extra_learn['sfwd_courses_archive_layout'] = array(
					'title'    => __( 'Course Archive Layout', 'codex' ),
					'panel'    => 'learndash',
					'priority' => 11,
				);
				...
				$extra_learn['sfwd_design'] = array(
					'title'    => __( 'Learn Dash Design', 'codex' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
				$extra_learn['sfwd_grid_layout'] = array(
					'title'    => __( 'Course Grid Design', 'codex' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
			...
			if ( class_exists( 'LifterLMS' ) ) {
				...
				$extra_lifter['llms_membership_layout'] = array(
					'title'    => __( 'Membership Layout', 'codex' ),
					'panel'    => 'lifterlms',
					'priority' => 14,
				);
				$extra_lifter['llms_membership_archive'] = array(
					'title'    => __( 'Membership Archive', 'codex' ),
					'panel'    => 'lifterlms',
					'priority' => 15,
				);
			...
			if ( defined( 'TRIBE_EVENTS_FILE' ) ) {
				$extra_events['tribe_events_layout'] = array(
					'title'    => __( 'Single Event Layout', 'codex' ),
			...
			$post_types        = codex()->get_post_types_objects();
			...
			foreach ( $post_types as $post_type_item ) {
				...
				if ( ! in_array( $post_type_name, $ignore_type, true ) ) {
					...
					$extras_post_types[ $post_type_name . '_layout' ] = array(
						'title'    => $post_type_label . ' ' . __( 'Layout', 'codex' ),
						'panel'    => $panel_name,
					...
					if ( $post_type_name !== 'ld-exam' ) {
						$extras_post_types[ $post_type_name . '_archive' ] = array(
							'title'    => $post_type_label . ' ' . __( 'Archive', 'codex' ),
							'panel'    => $panel_name,
							'priority' => $panel_priority,
						);
			...
			self::$sections = apply_filters( 'codex_theme_customizer_sections', $final_sections );
		}
		// Return sections.
	...
	public static function get_capability() {
		...
		if ( is_null( self::$capability ) ) {
			self::$capability = apply_filters( 'codex_theme_customizer_capability', 'manage_options' );
		}
		// Return capability.
	...
	public function register_settings( $wp_customize ) {
		...
		foreach ( self::$settings as $setting_key => $setting ) {
			...
			if ( isset( $setting['control_type'] ) && ! empty( $setting['control_type'] ) ) {
				if ( substr( $setting['control_type'], 0, 8 ) === 'codex_' ) {
					$control_class = 'codex_Control_' . ucfirst( str_replace( 'codex_', '', str_replace( '_control', '', $setting['control_type'] ) ) );
				} else {
			...
			if ( isset( $setting['sanitize'] ) && ! empty( $setting['sanitize'] ) ) {
				if ( substr( $setting['sanitize'], 0, 8 ) === 'codex_' ) {
					$sanitize_function = array( 'codex\Customizer_Sanitize', $setting['sanitize'] );
				} else {
					$sanitize_function = $setting['sanitize'];
				}
			} else {
				$sanitize_function = array( 'codex\Customizer_Sanitize', 'codex_sanitize_option' );
				$sanitize_function = '';
			}
			if ( 'option' === codex()->get_option_type() ) {
				$setting_key = codex()->get_option_name() . '[' . $setting_key . ']';
			}
			if ( ! empty( $opt_name_key = apply_filters( 'codex_settings_key_custom_mapping', '', $setting_key ) ) ) {
			...
			if ( ! in_array( $section_key, self::$sections_added, true ) ) {
				...
				if ( ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION, '10.6.2', '>' ) ) || version_compare( substr( get_bloginfo( 'version' ), 0, 3 ), '5.8', '>=' ) ) {
					if ( 'sidebar-widgets-footer1' === $section_key || 'sidebar-widgets-footer2' === $section_key || 'sidebar-widgets-footer3' === $section_key || 'sidebar-widgets-footer4' === $section_key || 'sidebar-widgets-footer5' === $section_key || 'sidebar-widgets-footer6' === $section_key || 'sidebar-widgets-header1' === $section_key || 'sidebar-widgets-header2' === $section_key || 'sidebar-widgets-product-filter' === $section_key ) {
						$section_id = 'codex_customizer_' . $section_key;
					}
				}
				// Register section.
	...
	public static function maybe_add_panel( $wp_customize, $child ) {
		...
		if ( ! empty( $child['panel'] ) && isset( $panels[ $child['panel'] ] ) ) {
			...
			if ( ! in_array( $panel_key, self::$panels_added, true ) ) {
				...
				// Register panel.
				$wp_customize->add_panel( 'codex_customizer_' . $panel_key, $panel_config );
		...
		foreach ( self::$settings as $setting_key => $setting ) {
			...
			if ( ! empty( $setting['section'] ) ) {
				...
				if ( ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION, '10.6.2', '>' ) ) || version_compare( substr( get_bloginfo( 'version' ), 0, 3 ), '5.8', '>=' ) ) {
					if ( 'sidebar-widgets-footer1' === $setting['section'] || 'sidebar-widgets-footer2' === $setting['section'] || 'sidebar-widgets-footer3' === $setting['section'] || 'sidebar-widgets-footer4' === $setting['section'] || 'sidebar-widgets-footer5' === $setting['section'] || 'sidebar-widgets-footer6' === $setting['section'] || 'sidebar-widgets-header1' === $setting['section'] || 'sidebar-widgets-header2' === $setting['section'] || 'sidebar-widgets-product-filter' === $setting['section'] ) {
						$control_config['section'] = 'codex_customizer_' . $setting['section'];
					}
				}
			}
			...
			if ( ! empty( $setting['panel'] ) ) {
				if ( 'woocommerce' === $control_config['panel'] ) {
					...
					$control_config['panel'] = 'codex_customizer_' . $setting['panel'];
				}
			}
	...
	public static function active_callback( $object ) {
		if ( ! isset( $object->setting->id ) && 'codex_builder_control' !== $object->type ) {
			return true;
		}
		if ( 'codex_builder_control' === $object->type ) {
			$id = str_replace( codex()->get_option_name() . '_', '', $object->id );
		} else {
		...
		if ( ! isset( $settings[ $id ]['active_callback'][0] ) && 3 === count( $settings[ $id ]['active_callback'] ) ) {
			...
			$field_value = codex()->option( $field_id );
	...
	public static function get_builder_choices() {
		return apply_filters( 'codex_theme_customizer_control_choices', self::$choices );
	}
	/**
	...
	public function enqueue_customizer_scripts() {
		...
		if ( isset( $_GET['wpum_email_customizer'] ) || apply_filters( 'prevent_codex_customizer_scripts', false ) ) {
			return;
		}
		...
		wp_enqueue_style( 'codex-customizer-styles', $path . 'css/codex-customizer.css', false, codex_VERSION );
		if ( is_rtl() ) {
			wp_enqueue_style( 'codex-customizer-rtl', $path . 'css/rtl-codex-customizer.css', false, codex_VERSION );
		}
		...
		wp_enqueue_script( 'customizer-text-widgets', get_template_directory_uri() . '/assets/js/text-widgets.min.js', array( 'jquery', 'backbone', 'editor', 'wp-util', 'wp-a11y', 'text-widgets' ), codex_VERSION, false );
		wp_enqueue_script( 'codex-customizer-controls', get_template_directory_uri() . '/assets/js/admin/customizer.js', $editor_dependencies, codex_VERSION, true );
		wp_enqueue_style( 'codex-customizer-controls', $path . 'react/build/controls.css', array( 'wp-components' ), codex_VERSION );
		wp_localize_script(
			'codex-customizer-controls',
			'codexCustomizerControlsData',
			array(
				'contexts'           => self::get_control_contexts(),
				'choices'            => self::get_builder_choices(),
				'palette'            => codex()->get_palette_for_customizer(),
				'gfontvars'          => $google_font_variants,
				'cfontvars'          => apply_filters( 'codex_theme_custom_fonts', array() ),
				'palettePresets'     => apply_filters( 'codex_theme_palette_presets', $palette_presets ),
				'flushFonts'         => wp_create_nonce( 'codex-local-fonts-flush' ),
				'fontPairs'          => apply_filters( 'codex_font_pairs_array', $fonts ),
				'source'             => apply_filters( 'codex_settings_extra_source', false ),
		...
		$css = ':root {
			--global-palette1: ' . codex()->palette_option( 'palette1' ) . ';
			--global-palette2: ' . codex()->palette_option( 'palette2' ) . ';
			--global-palette3: ' . codex()->palette_option( 'palette3' ) . ';
			--global-palette4: ' . codex()->palette_option( 'palette4' ) . ';
			--global-palette5: ' . codex()->palette_option( 'palette5' ) . ';
			--global-palette6: ' . codex()->palette_option( 'palette6' ) . ';
			--global-palette7: ' . codex()->palette_option( 'palette7' ) . ';
			--global-palette8: ' . codex()->palette_option( 'palette8' ) . ';
			--global-palette9: ' . codex()->palette_option( 'palette9' ) . ';
			--global-palette-highlight: ' . $this->render_color( codex()->sub_option( 'link_color', 'highlight' ) ) . ';
			--global-palette-highlight-alt: ' . $this->render_color( codex()->sub_option( 'link_color', 'highlight-alt' ) ) . ';
			--global-palette-highlight-alt2: ' . $this->render_color( codex()->sub_option( 'link_color', 'highlight-alt2' ) ) . ';
			--global-palette-btn: ' . $this->render_color( codex()->sub_option( 'buttons_color', 'color' ) ) . ';
			--global-palette-btn-hover: ' . $this->render_color( codex()->sub_option( 'buttons_color', 'hover' ) ) . ';
			--global-palette-btn-bg: ' . $this->render_color_or_gradient( codex()->sub_option( 'buttons_background', 'color' ) ) . ';
			--global-palette-btn-bg-hover: ' . $this->render_color_or_gradient( codex()->sub_option( 'buttons_background', 'hover' ) ) . ';
			--global-base-font: ' . codex()->sub_option( 'base_font', 'family' ) . ';
			--global-heading-font: ' . ( 'inherit' !== codex()->sub_option( 'heading_font', 'family' ) ? codex()->sub_option( 'heading_font', 'family' ) : 'var(--global-base-font)' ) . ';
		}';
		wp_add_inline_style( 'codex-customizer-controls', wp_strip_all_tags( $css ) );
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'codex-customizer-controls', 'codex' );
		}
	}
	/**
	...
	public function action_enqueue_customize_preview_scripts() {
		$path = get_template_directory_uri() . '/inc/customizer/';
		wp_enqueue_style( 'codex-customizer-preview', $path . 'css/codex-customizer-preview.css', false, codex_VERSION );
		wp_enqueue_script( 'codex-webfont-js', 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js', array( 'jquery' ), '1.6.26', true );
		wp_enqueue_script( 'codex-customizer-preview', $path . 'js/codex-customizer-preview.min.js', array( 'customize-preview' ), codex_VERSION, true );
		wp_localize_script(
			'codex-customizer-preview',
			'codexCustomizerPreviewData',
			array(
	...
	public static function get_control_contexts() {
		// Return contexts.
		return apply_filters( 'codex_theme_customizer_control_contexts', self::$contexts );
	}
	...
	public static function get_live_control_data() {
		// Return contexts.
		return apply_filters( 'codex_theme_customizer_live_control_data', self::$live_control );
	}
