...
(function() {
	...
	window.codexSlide = {
		...
		start: function( element ) {
			...
			if ( ! nextLabel ) {
				nextLabel = codexSlideConfig.next;
			}
			if ( ! prevLabel ) {
				prevLabel = codexSlideConfig.next;
			}
			if ( ! slideLabel ) {
				slideLabel = codexSlideConfig.slide;
			}
			...
			var slider = tns( {
				...
				ofLabel: codexSlideConfig.of,
				toLabel: codexSlideConfig.to,
		...
		initAll: function( element ) {
			document.querySelectorAll( '.codex-slide-init' ).forEach(function ( element ) {
				window.codexSlide.start( element );
			} );
		},
		// Initiate the menus when the DOM loads.
		init: function() {
			if ( typeof tns == 'function' ) {
				window.codexSlide.initAll();
			} else {
				var initLoadDelay = setInterval( function(){ if ( typeof tns == 'function' ) { window.codexSlide.initAll(); clearInterval(initLoadDelay); } }, 200 );
			}
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.codexSlide.init );
	} else {
		// The DOM has already been loaded.
		window.codexSlide.init();
	}
})();
