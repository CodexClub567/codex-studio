...
class ColorComponent extends Component {
	constructor(props) {
		...
		let baseDefault =  ( codexCustomizerControlsData.palette ? JSON.parse( codexCustomizerControlsData.palette ) : { palette: [] } );
		...
		let defaultParams = {
			...
			palettes: ( codexCustomizerControlsData.palettePresets ? codexCustomizerControlsData.palettePresets : [] ),
			colors: {
				palette1: {
					tooltip: __( '1 - Accent', 'codex' ),
					palette: false,
				},
				palette2: {
					tooltip: __( '2 - Accent - alt', 'codex' ),
					palette: false,
				},
				palette3: {
					tooltip: __( '3 - Strongest text', 'codex' ),
					palette: false,
				},
				palette4: {
					tooltip: __( '4 - Strong Text', 'codex' ),
					palette: false,
				},
				palette5: {
					tooltip: __( '5 - Medium text', 'codex' ),
					palette: false,
				},
				palette6: {
					tooltip: __( '6 - Subtle Text', 'codex' ),
					palette: false,
				},
				palette7: {
					tooltip: __( '7 - Subtle Background', 'codex' ),
					palette: false,
				},
				palette8: {
					tooltip: __( '8 - Lighter Background', 'codex' ),
					palette: false,
				},
				palette9: {
					tooltip: __( '9 - White or offwhite', 'codex' ),
					palette: false,
				},
			},
			paletteMap: {
				'palette': {
					'tooltip': __( 'Palette 1', 'codex' ),
				},
				'second-palette': {
					'tooltip': __( 'Palette 2', 'codex' ),
				},
				'third-palette': {
					'tooltip': __( 'Palette 3', 'codex' ),
				}
			}
		};
	...
	render() {
		...
		return (
				<div className="codex-control-field codex-palette-control codex-color-control">
					<div className="codex-palette-header">
					...
					<div ref={ this.anchorNodeRef } className="codex-palette-colors">
					...
					<div className={'codex-palette-import-wrap'}>
						<Button className={ 'codex-palette-import' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<Dashicon icon="portfolio" />
						</Button>
						{ this.state.isVisible && (
							<Popover position="bottom right" inline={true} className="codex-palette-popover-copy-paste codex-customizer-popover" onClose={ toggleClose }>
								<TabPanel className="codex-palette-popover-tabs"
									activeClass="active-tab"
									initialTabName={ 'import'}
									tabs={ [
										{
											name: 'import',
											title: __( 'Select Palette', 'codex' ),
											className: 'codex-color-presets',
										},
										{
											name: 'custom',
											title: __( 'Export/Import', 'codex' ),
											className: 'codex-export-import',
										}
									] }>
									{
										( tab ) => {
											...
											if ( tab.name ) {
												if ( 'import' === tab.name ) {
																	...
																	>
																		{ Object.keys( presetPalettes[item] ).map( ( color, subIndex ) => {
																			return (
																				...
																				} } className="codex-swatche-item-wrap">
																					<span
																						className={ 'codex-swatch-item' }
																						style={ {
																							height: '100%',
																							display: 'block',
																							width: '100%',
													...
													tabout = (
														<Fragment>
															<h2>{ __('Export', 'codex' ) }</h2>
															<TextareaControl
																label=""
																help={__("Copy export data to use in another site.", 'codex')}
																value={ currentPaletteJson }
																onChange={ false }
															/>
															<h2>{ __('Import', 'codex' ) }</h2>
															<TextareaControl
																label={__("Import color set from text data.", 'codex')}
																help={__("Follow format from export above.", 'codex')}
																value={ this.state.textImport }
																onChange={ ( text ) => this.setState( { textImport: text } ) }
															/>
															{ this.state.importError && (
																<p style={{color:'red'}}>{ __( 'Error with Import data', 'codex') }</p>
															) }
															<Button
																className={ 'codex-import-button' }
																isPrimary
					...
					{ this.props.control.params.description && (
						<span className="customize-control-description"><a href="https://codex-theme.com/docs/how-to-use-the-codex-color-palette/" target="_blank">{ this.props.control.params.description }</a></span>
					) }
				</div>
		);
	}
	...
	updateValues( value ) {
		...
		codexCustomizerControlsData.palette = JSON.stringify( value );
	}
}
